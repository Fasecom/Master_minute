<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4">
            <form method="POST" action="{{ route('shops.update', $shop->id) }}" class="flex flex-col gap-5 bg-white rounded-[21px] shadow-[0_5px_27px_rgba(46,69,85,0.25)] min-h-[340px] w-full" id="edit-shop-form">
                @csrf
                ...
                <div class="flex flex-row gap-8">
                    ...
                    <div class="flex flex-col gap-5 w-1/2 p-10" style="padding:30px;">
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="text" name="name" class="input-header w-full mb-1" placeholder="ТРЦ Фокус" value="{{ old('name', $shop->name) }}" required autocomplete="off">
                            ...
                            <input type="date" name="open_date" class="input-header w-full mb-1" value="{{ old('open_date', $shop->open_date ? date('Y-m-d', strtotime($shop->open_date)) : date('Y-m-d')) }}" required autocomplete="off">
                            ...
                            <div class="flex gap-5 mb-1">
                                <input type="time" name="open_time" class="input-header w-full" value="{{ old('open_time', $shop->open_time) }}" required autocomplete="off">
                                <input type="time" name="close_time" class="input-header w-full" value="{{ old('close_time', $shop->close_time) }}" required autocomplete="off">
                            </div>
                            ...
                            <input type="text" name="address" class="input-header w-full mb-1" placeholder="Молдавская улица, 16" value="{{ old('address', $shop->address) }}" required autocomplete="off">
                            ...
                            <input type="email" name="email" class="input-header w-full mb-1" placeholder="ivanivanovich@mail.ru" value="{{ old('email', $shop->email) }}" autocomplete="off">
                            ...
                            <input type="text" name="phone" class="input-header w-full mb-1" placeholder="+7 000 000 00 00" value="{{ old('phone', $shop->phone ? '+7 '.substr($shop->phone, 1, 3).' '.substr($shop->phone, 4, 3).' '.substr($shop->phone, 7, 2).' '.substr($shop->phone, 9, 2) : '') }}" id="phone-input" autocomplete="off">
                                        ...
                                        <div class="flex flex-col">
                                            @foreach($chunk as $service)
                                                <label class="flex items-center gap-2 mb-1">
                                                    <input type="checkbox" name="services[]" value="{{ $service->id }}" {{ (is_array(old('services', $shopServices)) && in_array($service->id, old('services', $shopServices))) ? 'checked' : '' }}>
                                                    {{ $service->name }}
                                                </label>
                                            @endforeach
                                        </div>
            ...
            <div id="modal-delete" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
                <div class="bg-white rounded-xl p-8 min-w-[320px] flex flex-col items-center">
                    <div class="mb-4 text-xl font-semibold text-[#DE5B5F]">Удалить торговую точку?</div>
                    <div class="flex gap-4 mt-2">
    ...
    function submitEditForm() {
        document.getElementById('edit-shop-form').submit();
    }
    function submitDeleteForm() {
        // Создаём и отправляем форму для удаления
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '{{ route('shops.delete', $shop->id) }}';
        form.innerHTML = '@csrf';
