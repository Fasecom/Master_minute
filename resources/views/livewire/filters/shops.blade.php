@php
    ...
    $display = $placeholder;
    if(count($selected)){
        $first = collect($options)->firstWhere('id',$selected[0]);
    ...
    <div class="absolute z-50 mt-2 bg-white rounded-md shadow-lg p-4 w-[300px] max-h-60 overflow-y-auto" wire:click.away="close">
        @foreach($options as $option)
            <label class="flex items-center gap-2 py-1 cursor-pointer hover:bg-gray-50">
                <input type="checkbox" value="{{ $option->id }}" @if(in_array($option->id, $selected)) checked @endif wire:click="toggleShop({{ $option->id }})" class="rounded border-gray-300">
                <span class="select-none">{{ $option->name }}</span>
            </label>
        @endforeach
        <button type="button" class="mt-2 btn-leval4 btn-red" style="max-width: 250px; width: 100%;" wire:click="resetShops">Сбросить</button>
    </div>
    @endif
</div> 