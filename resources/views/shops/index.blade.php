            ...
            <div class="flex flex-wrap items-center justify-between">
                ...
                <div class="flex items-center gap-4 ml-auto">
                    ...
                    @if(auth()->user()->role_id != 3)
                        <a href="{{ route('shops.analytics') }}" class="btn-leval1 flex items-center justify-center w-[240px] h-[44px]">
                            Аналитика
                        </a>
                        <a href="{{ route('shops.add') }}" class="btn-leval1 flex items-center justify-center w-[65px] h-[44px] rounded-full p-0" style="min-width:65px;min-height:44px;">
            ...
            <div class="flex flex-wrap gap-5 justify-center">
                @foreach ($shops as $shop)
                    ...
                    <div class="flex flex-col justify-between rounded-[21px] shadow-[0_5px_27px_rgba(46,69,85,0.25)] p-5 bg-white min-w-[400px] max-w-none min-[1271px]:max-w-[400px] flex-1 basis-[400px] h-[140px]">
                        <h3 class="h3-main mb-2" name="name">{{ $shop->name }}</h3>
                        <div class="flex items-center justify-between mt-auto">
                            <div class="flex items-center gap-2">
                                <img src="/img/icon/user.svg" alt="place" width="20" height="20">
                                <h4 class="h4-main">
                                    {{ $currentEmployee ? collect(explode(' ', $currentEmployee->full_name))->take(2)->implode(' ') : 'Нет сотрудника' }}
                                </h4>
                            </div>
                            <a href="{{ route('shops.info', ['id' => $shop->id]) }}" class="btn-leval2 btn-blue">Подробнее</a>
                        </div>
                    </div>
                @endforeach
            </div>
