<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4">
            <form method="POST" action="{{ route('masters.store') }}" class="bg-white rounded-[21px] shadow-[0_5px_27px_rgba(46,69,85,0.25)] flex flex-row min-h-[340px] w-full" id="add-master-form">
                @csrf
                <!-- Левая часть -->
                <div class="flex flex-col gap-5 w-1/2 p-10" style="padding:30px;">
                    <div class="flex flex-col gap-2">
                        ...
                        <input type="text" name="full_name" class="input-header w-full mb-1" placeholder="Иван Иванов" value="{{ old('full_name') }}" required autocomplete="off">
                        ...
                        <input type="date" name="work_start_date" class="input-header w-full mb-1" value="{{ old('work_start_date', date('Y-m-d')) }}" required autocomplete="off">
                        ...
                        <input type="text" name="phone" class="input-header w-full mb-1" placeholder="+7 000 000 00 00" value="{{ old('phone') }}" required id="phone-input" autocomplete="off">
                        ...
                        <div class="relative">
                            <input type="password" name="password" class="input-header w-full mb-1 pr-10" placeholder="*********" required id="password-input" autocomplete="new-password">
                                    ...
                                    <div class="flex flex-col">
                                        @foreach($chunk as $skill)
                                            <label class="flex items-center gap-2 mb-1">
                                                <input type="checkbox" name="skills[]" value="{{ $skill->id }}" {{ (is_array(old('skills')) && in_array($skill->id, old('skills', []))) ? 'checked' : '' }}>
                                                {{ $skill->name }}
                                            </label>
                                        @endforeach
                                    </div>
