...
<script>
    ...
    function {{ $fn }}(newData = null){
        // 1. Ждём, пока библиотека amCharts будет загружена
        if(!window.am5){ setTimeout(()=>{{ $fn }}(newData),100); return; }

        // 2. Данные для построения графика
        const data = newData ?? @json($chartData);

        // 3. Контейнер и плейсхолдер «Нет данных»
        ...
        // 6. Оси – категория по X, значения по Y (для вертикальных столбцов)
        ...
        // эффект наведения как в рейтинге: расширяем колонку
        ...
        // Цвет в зависимости от знака значения (плюс – синий, минус – красный)
        ...
        // курсор для наведения, как в графиках рейтинга
    ...
    // Слушаем обновление от Livewire
    ...
    // Перерисовываем при навигации Livewire
